...
PRIMARY_FONT_COLOR = "#b3c1e1"
SECONDARY_FONT_COLOR = "#fcc331"
PRIMARY_BG_COLOR = "#0f1f44"

mpl.rcParams['text.color'] = PRIMARY_FONT_COLOR
mpl.rcParams['axes.labelcolor'] = PRIMARY_FONT_COLOR
mpl.rcParams['xtick.color'] = PRIMARY_FONT_COLOR
mpl.rcParams['ytick.color'] = PRIMARY_FONT_COLOR
...
class _BarChartRace:
    ...
    def create_figure(self, dpi):
        ...
        fig.set_facecolor(PRIMARY_BG_COLOR)
        ...
        if self.orientation == 'h':
            ...
            ax.grid(False)
            ax.set_xticklabels(ax.get_xticklabels(), ha='right', rotation=30, font=REGULAR_FONT_FAMILY)
        ...
        ax.set_facecolor(PRIMARY_BG_COLOR)
        ax.set_title(self.title, size=self.title_size, font=BOLD_FONT_FAMILY, color=SECONDARY_FONT_COLOR)
        for spine in ax.spines.values():
            spine.set_visible(False)

        ax.set_xticklabels(ax.get_xticklabels(), font=REGULAR_FONT_FAMILY)
        ax.set_yticklabels(ax.get_yticklabels(), font=REGULAR_FONT_FAMILY)
        return fig, ax
    ...
    def calculate_new_figsize(self, real_fig):
        ...
        if self.orientation == 'h':
            ...
            ax.set_yticklabels(self.df_values.columns, font=REGULAR_FONT_FAMILY)
            ax.set_xticklabels([max_val] * len(ax.get_xticks()), font=REGULAR_FONT_FAMILY)
        else:
            ...
            fig.canvas.print_figure(io.BytesIO())
            orig_pos = ax.get_position()
            ax.set_xticklabels(self.df_values.columns, ha='right', font=REGULAR_FONT_FAMILY)
            ax.set_yticklabels([max_val] * len(ax.get_yticks()), font=REGULAR_FONT_FAMILY)
    ...
    def plot_bars(self, i):
        ...
        if self.period_label:
            ...
            if num_texts == 0:
                # first frame
                self.ax.text(s=s, transform=self.ax.transAxes, **self.period_label, font=REGULAR_FONT_FAMILY)
            else:
